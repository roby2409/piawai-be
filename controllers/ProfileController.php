...
class ProfileController
{
    ...
    // GET /profile/{username}  → profil orang lain (auth required)
    public function getByUsername(string $username): void
    {
        $stmt = $this->db->prepare("
            SELECT p.username, p.full_name, p.avatar_url, p.bio,
                   p.skills, p.lat, p.lng, p.is_available, p.age, p.phone_wa
            FROM profiles p
            WHERE p.username = ?
        ");
    ...
    // POST/PUT /profile  → update profil (auth required)
    // username tidak bisa diubah di sini, sudah diset saat register
    public function createOrUpdate(array $user): void
    {
        $body = $this->getBody();

        $fields = ['full_name', 'avatar_url', 'phone_wa', 'bio', 'skills', 'lat', 'lng', 'is_available', 'age'];
        $data   = [];
        ...
        // Encode skills ke JSON jika array
        if (isset($data['skills']) && is_array($data['skills'])) {
            $data['skills'] = json_encode($data['skills']);
        }

        // Profil sudah pasti ada (dibuat otomatis saat register)
        // tapi tetap handle insert untuk jaga-jaga (misal user lama / Google login)
